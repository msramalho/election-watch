  ...
  <v-container fill-height>
    <v-row no-gutters>
      <v-col cols="12" sm="12" md="6" lg="3">
        <v-card class="mx-auto" max-width="500" :loading="$fetchState.pending?'primary': false">
          <h3 v-if="$fetchState.pending" class="text-center ma-4">Loading...</h3>
          <h3 v-if="$fetchState.error" class="red--text text-center">Could not load logs</h3>
                    ...
                    <v-list-item-title>
                      <v-tooltip top v-if="log.elapsed">
                        <template v-slot:activator="{ on }">
                          <v-icon v-on="on" color="green" size="small">mdi-circle</v-icon>
                        </template>
                        <span>
                          running for {{log.elapsed[1]}}
                          <br />
                          started at {{log.elapsed[0]}}
                        </span>
                      </v-tooltip>
                      <span>
                        <span class="text--disabled">{{ task.split('/')[0] }}</span>
                        /{{ task.split('/')[1] }}
                      </span>
                    </v-list-item-title>
                  </v-list-item-content>
      ...
      <v-col cols="12" sm="12" md="6" lg="9" class="mt-sm-10 mt-md-0">
        <v-card
          ...
          max-width="100%"
        >
          <h3 v-if="selected.loading" class="text-center ma-4">Loading...</h3>
          <!-- <h3 v-if=".error" class="red--text text-center">Could not load logs</h3> -->
          <div v-else>
            <h3 class="text-center mb-3">
              <span class="text--disabled text-center">{{ selected.task }}</span>
              /
              {{selected.file}}
            </h3>
